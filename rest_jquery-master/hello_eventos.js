$(document).ready(function() {
    var trHTML = '';
    url_get = "http://localhost:5000/api/eventos"
        console.log(url_get)
        $.ajax({
	type: "GET",
        url: url_get
		...
		$.each(data, function (i, item) {
        	console.log(i)
        	console.log(item)
        trHTML += '<tr><td>' + item["nombre"] + '</td><td>' + item["dependencia"]  + '</td><td>' + item["link"]  + '</td><td>' + item["fecha"]  + '</td><td>' + item["hora"]  + '</td><td>' + item["lugar"]  + '</td><td>' + item["resumen"]  + '</td><td>' + item["nombre_e"]  + '</td><td>' + item["correo_e"]  + '</td><td>' + item["palabras"] + '</td></tr>';
    });
